...
> **Примечание (2026-08-16):** это первоначальный дизайн. Реальная
> реализация местами разошлась: окно датасета 1.2с вместо 1с,
> декодирование/ресемпл через ffmpeg, а Golos скачивается через
> HuggingFace-зеркало `bonlime/golos-test` (официальные файловые ссылки
> Сбера sc.link мертвы). Актуальная пошаговая инструкция —
> [08-runbook](08-runbook.md), обоснование размеров —
> [09-dataset-sizing](09-dataset-sizing.md).
